...
MalignantCalls_tib <- read_tsv("../11_pDC_expr/11.3_MalignantCalls_Final.txt")
MalignantCalls_df <- data.frame(MalignantCalls_tib, row.names = "cell")
seu$bpdcn_sign_score <- MalignantCalls_df[colnames(seu),"bpdcn_sign_score"]
...
metadata_tib %>% arrange(mutation) %>%
  ggplot(aes(x = CellType, y = bpdcn_sign_score, color = mutation)) +
...
p2 <- metadata_tib %>%
  ggplot(aes(x = UMAP_1, y = UMAP_2, color = bpdcn_sign_score)) +
  geom_point() +
  scale_color_viridis() +
  ggtitle(label = pt) +
  theme_bw() +
...
pdf(paste0("13.5_", pt, "_", cutf(mut, d = "\\."), "_UMAPs.pdf"), height = 4.5, width = 20)
print(
