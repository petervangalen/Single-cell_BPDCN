...
setwd("~/DropboxMGB/Projects/Single-cell_BPDCN/AnalysisPeter/scBPDCN-analysis/12_Pt10_donor_host")
...
# Load Volker's donor / recipient calls (from his email 200710)
...
pdf("12.1.1_CellTypeProportions.pdf", width = 8, height = 6)
...
celltype_freq %>% mutate(CellType = factor(CellType, levels = rev(levels(CellType)))) %>%
  ...
  theme_bw() +
  theme(aspect.ratio = 0.5, panel.grid = element_blank(), axis.ticks = element_line(color = "black"),
...
pdf("12.1_UMAP_all_dimensions.pdf", width = 20, height = 4)
