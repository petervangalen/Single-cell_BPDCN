...
# Define all UV-associated mutations --------------------------------------------------------------
current_mut <- filter(genotyping_tables.tib, `Founder or progression mutation` != "Founder",
                      `CC>CT (UV-associated)` == "Yes" |	`TC>TT (UV-associated)` == "Yes" | `CC>TT (UV-associated` == "Yes") %>%
  .$Mutation %>% unique


# Classify malignant/normal cells -----------------------------------------------------------------

# The justification for this is described in 11.1_Classify_malignant_BPDCN.R
...
metadata_tib <- metadata_tib %>% dplyr::select(cell, orig.ident, CellType, bpdcn_score, Malignant_cell, all_of(current_mut), UV_mutations)


# Stats for text

# "Of all 4,263 cells with TC>TT, CC>CT, or CC>TT mutations, 4,100 (96.2%) were classified as malignant BPDCN cells"
metadata_tib %>% filter(UV_mutations == "mutant") %>% nrow
metadata_tib %>% filter(UV_mutations == "mutant") %>% .$Malignant_cell %>% table
4100/4263*100

# Visualize pDC prediction scores vs. BPDCN scores and mutations  ---------------------------------

# Retrieve random forest prediction scores to plot on x-axis
...
pDC_scores_tib <- pDC_scores_tib %>% rename(RF_pDC_score = "x....pDC..") %>%
  mutate(cell = gsub("^.*?\\.", "", cell))

pdf("11.2.1_SinaPlot.pdf", width = 10, height = 3.5)
p1 <- metadata_tib %>%
...
p2 <- metadata_tib %>% left_join(pDC_scores_tib) %>%
  mutate(orig.order = sample(nrow(metadata_tib))) %>% arrange(orig.order) %>%
  filter(UV_mutations != "no call") %>%
  ggplot(aes(x = RF_pDC_score, y = bpdcn_score, color = UV_mutations)) +
  geom_point(size = 0.5) +
  scale_color_manual(values = c("#b0c4de", "#ffa500")) +
  guides(colour = guide_legend(override.aes = list(size=3))) +
  theme_bw() +
...
# Visualize UV-associated mutations in normal and malignant cells ---------------------------------
...
plot_tib <- metadata_tib %>%
  ...
  mutate(Malignant_cell = paste0(gsub("_cell", "", Malignant_cell), " (n = ", formatC(sum(n), big.mark=","), ")")) %>%
  mutate(Malignant_cell = factor(Malignant_cell, levels = rev(unique(.$Malignant_cell))))
  
pdf("11.2.2_UV_in_Malignant_vs_Normal.pdf", width = 5, height = 6)
plot_tib %>% filter(UV_mutations != "no call") %>%
  ...
  theme(aspect.ratio = 3, panel.grid = element_blank(), axis.ticks = element_line(color = "black"),
        axis.text = element_text(color = "black"), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
